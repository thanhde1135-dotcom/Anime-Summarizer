...
st.set_page_config(page_title="AI Lồng Tiếng Free", page_icon="🎬", layout="centered")
st.title("🎬 AI Lồng Tiếng + Dịch Phụ Đề (Bản Free)")
st.caption("Hoàn toàn miễn phí - không cần API key")
...
def transcribe_free(audio_path):
    """Nhận diện miễn phí bằng Google Speech Recognition"""
    recognizer = sr.Recognizer()
    segments = []
    
    # Chia nhỏ audio vì Google free có giới hạn độ dài
    audio = AudioSegment.from_wav(audio_path)
    chunk_length_ms = 50000  # 50 giây mỗi đoạn
    
    for i, start_ms in enumerate(range(0, len(audio), chunk_length_ms)):
        chunk = audio[start_ms:start_ms + chunk_length_ms]
        chunk_path = Path(tempfile.gettempdir()) / f"chunk_{i}.wav"
        chunk.export(chunk_path, format="wav")
        
        with sr.AudioFile(str(chunk_path)) as source:
            audio_data = recognizer.record(source)
            try:
                text = recognizer.recognize_google(audio_data, language="vi-VN")  # đổi nếu cần
                start = start_ms / 1000
                end = min((start_ms + chunk_length_ms) / 1000, len(audio) / 1000)
                segments.append({
                    "start": start,
                    "end": end,
                    "text": text
                ...
                st.warning(f"Lỗi nhận diện đoạn {i}: {e}")
...
if uploaded and st.button("Bắt đầu xử lý (Free)", type="primary"):
    with st.status("Đang xử lý bản Free...", expanded=True) as status:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            st.write("3. Đang nhận diện giọng nói (Google Free)...")
            segments = transcribe_free(audio_path)
            st.write(f"   → {len(segments)} đoạn")

            if not segments:
                st.error("Không nhận diện được giọng nói. Thử video rõ tiếng hơn.")
                st.stop()
            ...
            st.success("Xử lý thành công (bản Free)!")
            st.video(str(output_path))

            with open(output_path, "rb") as f:
                st.download_button("Tải video", f, file_name="dubbed_free.mp4")
