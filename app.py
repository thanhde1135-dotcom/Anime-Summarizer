...
st.sidebar.markdown("---")
st.sidebar.info(
    "💡 **Mẹo:** Nếu video quá dài, hãy dùng video ngắn dưới 2-3 phút hoặc cắt"
    " nhỏ đoạn cần tạo phụ đề."
)

# Khu vực tải lên file từ điện thoại/máy tính
uploaded_file = st.file_uploader(
    "📁 Tải lên Video hoặc Audio (Khuyên dùng file dưới 25MB)",
  ...
  if st.button(
    ...
    if not groq_api_key:
      ...
      with st.spinner(
        ...
        try:
          # Kiểm tra kích thước file (giới hạn Groq là 25MB)
          file_bytes = uploaded_file.read()
          file_size_mb = len(file_bytes) / (1024 * 1024)

          if file_size_mb > 25:
            st.error(
                f"❌ File của bạn nặng {file_size_mb:.1f}MB. API Groq chỉ"
                " nhận file tối đa 25MB. Vui lòng chọn video ngắn hơn hoặc nén"
                " file lại!"
            )
          else:
            # Lưu file tạm thời
            ...
            # Bước 1: Nhận diện giọng nói (Transcribe)
